...
chimera = Chimera(tolerances=[0.8, 0.1, 1.0], absolutes=[True, True, True], goals=['max', 'min', 'max'])

# --------------------
# load emulator model
# --------------------
with open('../emulator_train_data.pkl', 'rb') as content:
    df = pickle.load(content)

normalizer = preprocessing.Normalization()
normalizer.adapt(np.array(df.loc[:, ['c60_flow', 'sultine_flow', 'T']]))
...
def build_model():
    ...
    model = keras.Sequential([
          normalizer,
          layers.Dense(64, input_shape=[3], activation='relu', kernel_initializer=initializer),
    ...
    model.compile(loss=tf.keras.losses.CategoricalCrossentropy(from_logits=False), 
                  ...
                  metrics=[tf.keras.losses.CategoricalCrossentropy(from_logits=False, name='categorical_crossentropy')]
                  )
    return model

# Loads saved weights
model = build_model()
model.load_weights("../tf_prod_model/cp.ckpt")
...
for num_repeat in range(missing_repeats):
    ...
    while len(X_samples) < budget:
        ...
        offspring_objs = [eval_merit(ind) for ind in offspring]
