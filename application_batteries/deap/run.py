...
#chimera = Chimera(tolerances=[2.0, -0.7, 5.0], absolutes=[True, True, True], goals=['min', 'min', 'min'])
chimera = Chimera(tolerances=[25, 2.0383718, 0.0], absolutes=[True, True, False], goals=['min', 'min', 'min'])
...
def run_experiment(ind):
    match = df_results.loc[
                (df_results['r1_label'] == ind[0]) &
                (df_results['r3_label'] == ind[1]) &
                (df_results['r4_label'] == ind[2]) &
                (df_results['r5_label'] == ind[3])
            ]
    assert len(match) == 1
    abs_lam_diff = match.loc[:, 'abs_lam_diff'].to_numpy()[0]
    ered = match.loc[:, 'ered'].to_numpy()[0]
    gsol = match.loc[:, 'gsol'].to_numpy()[0]
    return abs_lam_diff, ered, gsol


def eval_merit(ind):
    abs_lam_diff, ered, gsol = run_experiment(param)
    return abs_lam_diff, ered, gsol
...
R1_LABELS = [i for i in df_results.loc[:, 'r1_label'].unique()]
R3_LABELS = [i for i in df_results.loc[:, 'r3_label'].unique()]
R4_LABELS = [i for i in df_results.loc[:, 'r4_label'].unique()]
R5_LABELS = [i for i in df_results.loc[:, 'r5_label'].unique()]

param_space = [
               {'type': 'categorical', 'categories': R1_LABELS},
               {'type': 'categorical', 'categories': R3_LABELS},
               {'type': 'categorical', 'categories': R4_LABELS},
               {'type': 'categorical', 'categories': R5_LABELS},
            ]
...
for num_repeat in range(missing_repeats):
    ...
    fitnesses = chimera.scalarize(np.array(y_samples))
    ...
    r1_label = [x[0] for x in X_samples]
    r3_label = [x[1] for x in X_samples]
    r4_label = [x[2] for x in X_samples]
    r5_label = [x[3] for x in X_samples]

    abs_lam_diff = [y[0] for y in y_samples]
    ered         = [y[1] for y in y_samples]
    gsol         = [y[2] for y in y_samples]
    data = pd.DataFrame({ 'r1_label': r1_label, 'r3_label': r3_label, 'r4_label': r4_label, 'r5_label': r5_label,  'abs_lam_diff': abs_lam_diff, 'ered': ered, 'gsol': gsol })
