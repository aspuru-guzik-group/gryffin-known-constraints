...
from benchmark_functions import DiscreteAckleyConstr as BenchmarkSurface
...
for num_repeat in range(missing_repeats):
    ...
    for ind, fit in zip(population, fitnesses):
        ...
        y_samples.append(fit[0])

    # create hall of fame
    num_elites = 2  # 2 elite individual in each population (i.e. always keep best)
    ...
    while len(X_samples) < budget:
        ...
        # if we are over the budget limit, stop the optimisation
        if len(X_samples) >= budget:
            break
